...
const STORE_LAST_SAMPLE_AT = 'boiler_energy_last_sample_at';
const STORE_LAST_POWER_W = 'boiler_energy_last_power_w';
const STORE_METER_KWH = 'boiler_energy_meter_kWh';
...
function integrateBoilerPower(previousPowerW, currentPowerW, elapsedMs) {
...
function calculateNextBoilerMeter({
    currentMeterKWh,
    previousPowerW,
    currentPowerW,
    previousSampleAt,
    ...
    return round(meter + integrateBoilerPower(previousPowerW, currentPowerW, elapsedMs));
}

async function updateEstimatedBoilerMeter(device, currentPowerW, now = Date.now()) {
    ...
    const nextMeterPower = calculateNextBoilerMeter({
        currentMeterKWh,
        previousPowerW,
        currentPowerW,
        previousSampleAt,
