...
const CONFIG = {
    RATE_LIMIT: 2000,
    MAX_LOG_SIZE: 500,
    MAX_AGE: 12 * 60 * 60 * 1000,
    CLEANUP_INTERVAL: 60 * 60 * 1000,
    LOG_BUFFER_SIZE: 50,
    LOG_BUFFER_TIMEOUT: 10000,
    MAX_KEY_LENGTH: 100,
    EXCLUDED_OBJECTS: ['Homey', 'MyApp', 'Device']
};

const SENSITIVE_KEY_PATTERN = /cookie|session|token|password|authorization|secret/i;
...
class Logger {
    ...
    constructor(homey, context) {
        ...
        this.#setupCleanup();
        this.info(`Logger initialized for context: ${context}`);
    }
        ...
        try {
            ...
            switch (logEntry.type) {
                ...
                case LOG_TYPES.WARNING:
                    this.#homey.log('[WARN]', message, logData);
                    break;
            ...
            this.#homey.error('Error while processing log entry:', error);
        }
    }
    ...
    clearHistory() {
        ...
        this.#logBuffer = [];
        this.#lastFlush = Date.now();
        this.info(`Log history cleared (${count} entries)`);
    }
    ...
    rotateLogs() {
        try {
            ...
            this.#homey.error("Error while rotating logs:", error);
        }
    }
