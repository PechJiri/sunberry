...
function formatConnectionError(error, host) {
    const code = getNestedErrorCode(error);
    if (code === 'ENOTFOUND') {
        return `Could not resolve ${host}. Try the actual Sunberry IP address.`;
    }

    if (code === 'ECONNREFUSED') {
        return `Could not connect to ${host}. Check that the Sunberry device is online and reachable.`;
    }
    ...
    return class SunberrySplitDriver extends HomeyDriver {
        ...
        async onPair(session) {
            ...
            session.setHandler('check', async (settings) => {
                let host = '';
                try {
                    host = normalizeHost(settings.ip_address);
                    if (!isValidHost(host)) {
                        throw new Error('Invalid IP address or hostname');
                    }
                    ...
                    return { success: false, error: formatConnectionError(error, host || settings.ip_address) };
                }
            });
