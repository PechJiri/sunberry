...
const STORE_LAST_SAMPLE_AT = 'energy_balance_last_sample_at';
const STORE_LAST_POWER_W = 'energy_balance_last_power_w';
const STORE_IMPORTED_KWH = 'energy_balance_imported_kWh';
const STORE_EXPORTED_KWH = 'energy_balance_exported_kWh';
...
function calculateNetGridPower({ homeConsumptionPowerW, batteryPowerW, solarPowerW }) {
    return round(
        numberOrZero(homeConsumptionPowerW)
        + numberOrZero(batteryPowerW)
        - numberOrZero(solarPowerW),
        2
    );
}

function calculateNextEnergyBalanceMeters({
    currentImportedKWh,
    currentExportedKWh,
    previousPowerW,
    currentPowerW,
...
async function updateEstimatedEnergyBalanceMeters(device, currentPowerW, now = Date.now()) {
    ...
    const nextMeters = calculateNextEnergyBalanceMeters({
        currentImportedKWh,
        currentExportedKWh,
        previousPowerW,
        currentPowerW,
