...
function parseBoilerValues(html) {
    ...
    let phaseCount = Object.values(phases).some(phase => phase.percent || phase.power) ? 3 : 0;
    const [singlePhasePercent, singlePhasePower] = pairAfterLabel(labels, /^V.*kon spir/i);
    if (!phaseCount && /%/.test(singlePhasePercent || '') && /W/i.test(singlePhasePower || '')) {
        phases.L1 = {
            percent: parseNumber(singlePhasePercent) || 0,
            power: parseNumber(singlePhasePower) || 0
        };
        phaseCount = 1;
    }
