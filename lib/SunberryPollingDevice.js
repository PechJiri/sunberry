...
const SELF_HEAL_POLLING_INTERVAL_MS = 60 * 60 * 1000;
...
class SunberryPollingDevice extends HomeyDevice {
    ...
    getNextPollingDelayMs() {
        if ((this.pollingFailureCount || 0) >= MAX_POLLING_FAILURES_BEFORE_UNAVAILABLE) {
            return SELF_HEAL_POLLING_INTERVAL_MS;
        }
    ...
    async markLastSeen() {
        if (typeof this.setLastSeenAt === 'function') {
            await this.setLastSeenAt(new Date());
        }
    }
    ...
    async setTransientWarning(error) {
        if (typeof this.setWarning === 'function') {
            await this.setWarning(`Sunberry polling failed: ${error.message}`);
        }
    }
